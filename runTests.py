...
def runTests(cover_file: Path, disk_file: Path):
    # Make tests directory if there is none
    if not isdir("tests"):
        mkdir("tests")
    ...
    for k, p in FRAG_PAIRS:
        ...
        df.to_csv("tests/data_{k}k_{p}p.csv")
...
def main() -> None:
    ...
    if not args.disk_file.is_file():
        ...
        exit(-1)
    runTests(cover_file=args.cover_file, disk_file=args.disk_file)
