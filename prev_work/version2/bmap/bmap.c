...
{
	if(argc==4 && !strcmp(argv[2], "slack"))
	{
		printf("Tool Usage:\nsudo <path_to_bmap_tool> --mode slack <path_to_cover_file> <num of bytes to retrieve>\n");
		printf("sudo /media/sf_Ubuntu_share/bmap/bmap --mode slack /usr/bin/unsquashfs 3796\n");
		exit(EXIT_FAILURE);
	}

	// if((argc==5) && ((atoi(argv[4]) < 0) || (atoi(argv[4]) > MAX_SLACK_BYTES)))
	// {
	// 	printf("<num of bytes> has to be +ve and <= %i! \n", MAX_SLACK_BYTES);
	// 	exit(EXIT_FAILURE);
	// }
...
union option_arg option_arg;
	...
	for(x=0;x<block_count;x++)
	{
		if((block=bmap_map_block(target_fd,x))==-1)
		{
			...
			mft_logf(MLOG_INFO,"null block while mapping block %d.",x);
	...
	{
		printf("slack_bytes is 0...line 429 of 'bmap.c'\n");
		slack_bytes=0;
	}
	else
	{
		slack_bytes=block_size-filesize%block_size;
		// printf("slack_bytes is: %i\n", slack_bytes );
	}
	...
	if(flag_mode==BMAP_SLACK && block!=0)
	{
		...
		if(lseek(raw_fd,offset,SEEK_SET)!=offset) {
			...
			int bytesRead = read(raw_fd,block_buffer,slack_bytes);
			if(strlen(block_buffer) < atoi(argv[4])){
				if(lseek(raw_fd,offset,SEEK_SET)==offset)
				{
					read(raw_fd,block_buffer,atoi(argv[4]));
					write(outfile_fd,block_buffer,atoi(argv[4]));
				}
			}
			else
				write(outfile_fd,block_buffer,bytesRead);
			fflush(stdout);//Added by Srinivasan
			fflush(stderr);//Added by Srinivasan
		}
		...
		if(lseek(raw_fd,offset,SEEK_SET)!=offset) {
			...
			// slack_bytes=read(0,block_buffer,atoi(argv[4]));// Added by Srinivasan
			// printf("slack_bytes: %d\n", slack_bytes);//Added by Srinivasan
			// printf("num bytes written: %d\n", strlen(block_buffer));//Added by Srinivasan
			write(raw_fd,block_buffer,slack_bytes);
			// write(raw_fd,block_buffer,atoi(argv[4]));// Added by Srinivasan
		}
